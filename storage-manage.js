...
/* Must match by data-load-id, not array position: the Load Board is filtered into
   folder tabs (Active/Completed/Paid/Archive), so a card's index among the VISIBLE
   cards no longer lines up with its index in the full, unfiltered loads array. Using
   the index alone opened the wrong load's documents whenever a folder was filtered. */
...
async function manageDocsStorage(card,i){
  ...
  modal.querySelectorAll('.zap-delete-doc').forEach(b=>b.onclick=async()=>{if(!confirm('Delete this document from the TMS list?'))return;const del=await sb.from('load_documents').delete().eq('id',b.dataset.id);if(del.error)return alert(del.error.message);alert('Document deleted.');manageDocsStorage(card,i)})
}
function hook(){document.querySelectorAll('#loadsList .list-card').forEach((card,i)=>{const b=card.querySelector('.zap-manage-docs-btn');if(b&&!b.dataset.storageManage){b.dataset.storageManage='1';b.onclick=()=>manageDocsStorage(card,i);}})}
