...
function markCards(){const arr=loads();qa('#loadsList .list-card').forEach((card,i)=>{const l=loadForCard(card,i,arr);if(!l)return;if(ok(l.status)){let box=card.querySelector('.invoice-select-box');if(!box){box=document.createElement('label');box.className='invoice-select-box';box.style.cssText='display:flex;gap:8px;align-items:center;margin-bottom:8px;font-weight:800;color:var(--green)';box.innerHTML='<input type="checkbox" class="invoice-select"> Select (invoice / mark paid / archive)';card.prepend(box)}box.querySelector('input').dataset.id=l.id}let acts=card.querySelector('.card-actions');if(!acts){acts=document.createElement('div');acts.className='card-actions';card.appendChild(acts)}if(!card.querySelector('.revoke-link-btn')){const b=document.createElement('button');b.className='small-btn revoke-link-btn';b.textContent='Revoke Link';b.onclick=()=>revokeLink(l.id);acts.appendChild(b)}})}
