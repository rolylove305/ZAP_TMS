...
/* load_documents rows created by the "Upload Rate Con" flow before a load exists yet
   (load_id null). Linked to whatever load the user saves next, so the Rate Con ends up
   attached to its load's documents instead of sitting orphaned in ratecon-inbox forever. */
let pendingRateConDocIds=[];
let accountType="dispatcher";
let currentProfileRole="user";
let currentOrganizationId=null;
let appData={settings:store.get("settings",{companyName:"Zap Dispatch",defaultCommission:8,companyEmail:"",companyPhone:""}),carriers:[],brokers:[],loads:[],expenses:[],carrier_charges:[],fleet_people:[]};
window.appData=appData;
const tables=["carriers","brokers","loads","expenses","carrier_charges","fleet_people"];
const map={
  carriers:{
    ...
    fromDb:x=>({id:x.id,name:x.name,mcDot:x.mc_dot,contact:x.contact,phone:x.phone,email:x.email,equipment:x.equipment,trucks:x.trucks,commission:x.commission,createdAt:x.created_at,organizationId:x.organization_id,linkedCarrierOrganizationId:x.linked_carrier_organization_id})
  },
  brokers:{
    ...
    fromDb:x=>({id:x.id,name:x.name,contact:x.contact,phone:x.phone,email:x.email,source:x.source,notes:x.notes,createdAt:x.created_at,organizationId:x.organization_id})
  },
  loads:{
  ...
  carrier_charges:{
    toDb:x=>({carrier_id:x.carrierId||null,carrier:x.carrier||"",carrier_organization_id:x.carrierOrganizationId||null,load_id:x.loadId||null,charge_date:emptyDate(x.date),category:x.category||"Other Service",description:x.description||"",amount:num(x.amount,0),status:x.status||"pending",invoice_id:x.invoiceId||null,attachment_bucket:x.attachmentBucket||"",attachment_path:x.attachmentPath||"",attachment_name:x.attachmentName||"",attachment_type:x.attachmentType||"",user_id:currentUser.id,...tenantField()}),
    fromDb:x=>({id:x.id,carrierId:x.carrier_id,carrier:x.carrier,carrierOrganizationId:x.carrier_organization_id,loadId:x.load_id,date:x.charge_date,category:x.category,description:x.description,amount:x.amount,status:x.status,invoiceId:x.invoice_id,attachmentBucket:x.attachment_bucket,attachmentPath:x.attachment_path,attachmentName:x.attachment_name,attachmentType:x.attachment_type,createdAt:x.created_at,organizationId:x.organization_id})
  },
  fleet_people:{
    toDb:x=>({person_type:x.personType||"company_driver",name:x.name||"",phone:x.phone||"",email:x.email||"",truck_number:x.truckNumber||"",trailer_number:x.trailerNumber||"",equipment:x.equipment||"",pay_type:x.payType||"per_mile",pay_rate:num(x.payRate,0),active:x.active!==false,notes:x.notes||"",eld_external_driver_id:x.eldExternalDriverId||null,user_id:currentUser.id,...tenantField()}),
    fromDb:x=>({id:x.id,personType:x.person_type,name:x.name,phone:x.phone,email:x.email,truckNumber:x.truck_number,trailerNumber:x.trailer_number,equipment:x.equipment,payType:x.pay_type,payRate:x.pay_rate,active:x.active,notes:x.notes,eldExternalDriverId:x.eld_external_driver_id,createdAt:x.created_at,organizationId:x.organization_id})
  }
};
...
async function insertRow(t,row){setBusy(true);try{const {data,error}=await sb.from(t).insert(map[t].toDb(row)).select().single();if(error)throw error;const saved=map[t].fromDb(data);appData[t]=[saved,...appData[t]];cache();refresh();const typeLabel=t==="loads"?"Load":t==="carriers"?"Carrier":t==="brokers"?"Broker":t==="expenses"?"Expense":t==="carrier_charges"?"Charge":t==="fleet_people"?"Driver":"Item";showToast(`✓ ${typeLabel} saved`,"success");return saved}catch(e){alert("Save error: "+e.message);return null}finally{setBusy(false)}}
...
function setSelectedLoadFolder(folder){
  ...
  ["zapSearch","zapStatus","zapCarrier"].forEach(id=>{const el=$(id);if(el)el.value=""});
  const note=$("zapFilterNote");if(note)note.textContent="";
  return next;
}
...
function normalizeAiStops(stops){
  return (Array.isArray(stops)?stops:[]).map(s=>s&&typeof s==="object"&&!Array.isArray(s)?{
    address:s.address?String(s.address).trim():"",
    num:s.num?String(s.num).trim():"",
    time:s.time?String(s.time).trim():"",
    date:s.date?String(s.date).trim():""
  }:null).filter(s=>s&&(s.address||s.num||s.time||s.date));
}
function fillNewLoadStopsFromAi(stops){
  const container=$("newLoadStops");
  if(!container)return 0;
  if(collectStops(container).length)return 0;
  const cleanStops=normalizeAiStops(stops);
  if(!cleanStops.length)return 0;
  initStopsEditor(container,$("addStopBtn"),cleanStops);
  return cleanStops.reduce((total,s)=>total+["address","num","time","date"].filter(k=>s[k]).length,0);
}
...
async function openChargeAttachment(c,btn){if(!c.attachmentPath)return;btn.disabled=true;try{const s=await sb.storage.from(c.attachmentBucket||"load-documents").createSignedUrl(c.attachmentPath,3600);if(s.error)throw s.error;window.open(s.data.signedUrl,"_blank")}catch(e){alert("Could not open backup: "+(e.message||String(e)))}finally{btn.disabled=false}}window.openChargeAttachment=openChargeAttachment;
async function voidCharge(id){if(!confirm("Void this charge? It will not be included in a future invoice."))return;const r=await sb.from("carrier_charges").update({status:"void"}).eq("id",id).select().single();if(r.error)return alert(r.error.message);await loadCloud()}window.voidCharge=voidCharge;
function renderCharges(){const list=$("chargesList");if(!list)return;const arr=data().carrier_charges||[];if($("chargeDate")&&!$("chargeDate").value)$("chargeDate").value=getTodayDate();renderChargeLoadOptions();const pendingRows=arr.filter(c=>String(c.status||"pending").toLowerCase()==="pending"),pending=pendingRows.reduce((s,c)=>s+num(c.amount),0);list.innerHTML='<div class="metric-card" style="margin-bottom:12px"><p>Pending additional fees</p><h3>'+money(pending)+'</h3><p class="muted" style="font-size:12px;margin:4px 0 0">'+pendingRows.length+' pending charge(s)</p></div>';if(!arr.length){list.innerHTML+="<div class='card'><p class='muted'>No additional charges yet.</p></div>";return}arr.forEach((c,i)=>{const status=String(c.status||"pending").toLowerCase(),color=status==="invoiced"?"green":status==="void"?"red":"yellow";const linkedLoad=c.loadId&&loadById(c.loadId);const scope=linkedLoad?loadLabel(linkedLoad):"Weekly / General";const backup=c.attachmentPath?'<button class="small-btn" onclick="openChargeAttachment(appData.carrier_charges['+i+'],this)">Open backup</button>':"";const actions=accountType==="dispatcher"?'<div class="card-actions">'+backup+(status==="pending"?'<button class="small-btn" onclick="voidCharge(\''+esc(c.id)+'\')">Void</button><button class="small-btn" onclick="removeItem(\'carrier_charges\','+i+')">Delete</button>':"")+'</div>':(backup?'<div class="card-actions">'+backup+'</div>':"");list.innerHTML+=card(`${c.category||"Other Service"} • ${money(c.amount)}`,`${c.carrier||"Carrier"} • ${scope} • ${c.date||""}${c.description?" • "+c.description:""}`,`<span class="pill ${color}">${esc(status)}</span><span class="pill">${esc(scope)}</span>${c.invoiceId?'<span class="pill">Invoiced</span>':""}${c.attachmentPath?'<span class="pill">Backup attached</span>':""}`,actions)})}
...
else if(ai.milesDebug&&!ai.milesDebug.startsWith("skipped: document already had miles"))carrierMatchNote+=(carrierMatchNote?"\n":"")+"⚠ Could not auto-calculate miles ("+ai.milesDebug+") — enter them manually.";
...
$("addLoad").onclick=addLoadFromForm;$("addFleetPerson").onclick=async()=>{if(window.zapPlanLimits&&!(await window.zapPlanLimits.requireFeature("fleetPeople")))return;addFleetPersonFromForm()};
...
async function startApp(){
  ...
  const profile=await sb.from("profiles").select("account_type,role,default_organization_id,plan").eq("id",currentUser.id).maybeSingle();
  ...
  if(window.zapPlanLimits){await window.zapPlanLimits.loadPlan();window.zapPlanLimits.renderBadge();}
  await migrateLocal();
  await loadCloud();
}
