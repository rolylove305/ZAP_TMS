...
function renderDashboard(){
  ...
  renderTodayAlerts();renderTodayMoney();renderTodayLoads();
}
...
function renderTodayAlerts(){const list=$("todayAlerts");if(!list)return;const delivered=getDeliveredWithoutInvoice();const pendingInv=delivered.length;list.innerHTML='<div class="metric-card"><p>Delivered, no invoice</p><h3>'+pendingInv+'</h3><button class="small-btn" style="margin-top:8px;width:100%" onclick="navTo(\'loads\')">Invoice now →</button></div>';list.innerHTML+='<div class="metric-card"><p>Delivered, no POD</p><h3>0</h3><button class="small-btn" style="margin-top:8px;width:100%" onclick="navTo(\'loads\')">Upload POD →</button></div>';}
function renderTodayMoney(){const list=$("todayMoney");if(!list)return;const pending=getPendingMoneyToInvoice();const d=data(),allComm=d.loads.reduce((s,l)=>s+(num(l.rate)*loadCommissionPct(l)/100),0);list.innerHTML='<div class="metric-card"><p>Pending to invoice</p><h3>'+money(pending)+'</h3><p class="muted" style="font-size:12px;margin:4px 0 0">'+getDeliveredWithoutInvoice().length+' loads</p></div>';list.innerHTML+='<div class="metric-card"><p>Dispatch fees (all)</p><h3>'+money(allComm)+'</h3><p class="muted" style="font-size:12px;margin:4px 0 0">'+d.loads.length+' loads</p></div>';}
function renderTodayLoads(){const list=$("todayLoads");if(!list)return;const loads=getTodayLoads();if(!loads.length){list.innerHTML='<p class="muted">No loads scheduled for today.</p>';return;}let html='<p style="font-size:13px;color:var(--text-secondary);margin:0 0 8px">Today\'s loads</p>';loads.forEach(l=>{const status=String(l.status||"Booked");const comm=num(l.rate)*loadCommissionPct(l)/100;html+='<div class="card" style="padding:1rem;margin-bottom:8px"><p style="font-weight:500;margin:0 0 6px;color:var(--text-primary);font-size:15px">'+esc((l.pickup||"Pickup")+" → "+(l.delivery||"Delivery"))+'</p><p style="font-size:13px;color:var(--text-secondary);margin:0">'+(accountType==="carrier"?esc((l.broker||"Broker")+" • "+(l.loadNumber||"-")):(l.carrier||"Carrier")+" • "+esc(l.loadNumber||"-"))+' • '+esc(status)+'</p><div style="display:flex;gap:6px;margin-top:8px;flex-wrap:wrap"><span class="pill">'+money(l.rate)+'</span>'+(accountType==="dispatcher"?'<span class="pill green">Comm '+money(comm)+'</span>':'')+'<span class="pill">'+esc(l.equipment||"")+'</span></div></div>';});list.innerHTML=html;}
...
$("addCarrier").onclick=addCarrierFromForm;
