...
const map={
  ...
  carrier_charges:{
    toDb:x=>({carrier_id:x.carrierId||null,carrier:x.carrier||"",carrier_organization_id:x.carrierOrganizationId||null,charge_date:emptyDate(x.date),category:x.category||"Other Service",description:x.description||"",amount:num(x.amount,0),status:x.status||"pending",invoice_id:x.invoiceId||null,attachment_bucket:x.attachmentBucket||"",attachment_path:x.attachmentPath||"",attachment_name:x.attachmentName||"",attachment_type:x.attachmentType||"",user_id:currentUser.id,...tenantField()}),
    fromDb:x=>({id:x.id,carrierId:x.carrier_id,carrier:x.carrier,carrierOrganizationId:x.carrier_organization_id,date:x.charge_date,category:x.category,description:x.description,amount:x.amount,status:x.status,invoiceId:x.invoice_id,attachmentBucket:x.attachment_bucket,attachmentPath:x.attachment_path,attachmentName:x.attachment_name,attachmentType:x.attachment_type,createdAt:x.created_at,organizationId:x.organization_id})
  },
  fleet_people:{
