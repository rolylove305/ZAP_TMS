(()=>{
  ...
  function planOptions(){
    return '<div style="display:grid;gap:8px;margin:18px 0;text-align:left">'
      +PLAN_ORDER.map(key=>{
        const p=PLANS[key];
        const checked=key===selectedPlan?' checked':'';
        return '<label style="display:grid;grid-template-columns:auto 1fr;gap:10px;align-items:start;border:1px solid rgba(148,163,184,.35);border-radius:10px;padding:12px;background:rgba(255,255,255,.04);cursor:pointer">'
          +'<input type="radio" name="zapPlan" value="'+esc(key)+'"'+checked+' style="margin-top:3px">'
          +'<span><b style="display:flex;justify-content:space-between;gap:12px"><span>'+esc(p.name)+'</span><span>'+esc(p.price)+'</span></b>'
          +'<small class="muted" style="display:block;margin-top:5px;line-height:1.45">'+esc(p.summary)+'</small></span>'
          +'</label>';
      }).join('')
      +'</div>';
  }
  ...
  function showPaywall(p){
    ...
    const buy=suspended?''
      :planOptions()
       +'<button class="primary-btn" id="zapSubBtn" style="width:100%;margin-top:8px">Subscribe — '+priceLabel(selectedPlan)+'</button>'
    ...
    el.querySelectorAll('input[name="zapPlan"]').forEach(input=>{
      input.onchange=()=>{
        selectedPlan=input.value;
        if(subBtn)subBtn.textContent='Subscribe — '+priceLabel(selectedPlan);
      };
    });
