...
async function deleteInvite(email,box){if(!confirm('Delete the invitation for '+email+'? They will no longer be able to log in or create an account. This removes the invitation permanently.'))return;const r=await sb.from('app_invites').delete().eq('email',email);if(r.error)return alert(r.error.message);loadInvites(box)}
async function loadInvites(box){
  ...
  box.innerHTML='<div style="display:flex;gap:8px;flex-wrap:wrap;align-items:center">'
    ...
    +'<button type="button" class="small-btn" id="inviteDeleteBtn" style="border-color:rgba(251,113,133,.4);color:#fda4af">Delete</button>'
    +'</div>';
  const sel=box.querySelector('#inviteSelect'),btn=box.querySelector('#inviteToggleBtn'),del=box.querySelector('#inviteDeleteBtn');
  const sync=()=>{const x=inv[+sel.value];const isOwner=String(x.email).toLowerCase()===OWNER;btn.textContent=x.active?'Turn off':'Turn on';btn.disabled=isOwner;del.disabled=isOwner;btn.title=del.title=isOwner?'The owner account cannot be changed':''};
  sel.onchange=sync;sync();
  btn.onclick=()=>{const x=inv[+sel.value];toggleInvite(x.email,x.active,box)};
  del.onclick=()=>{const x=inv[+sel.value];deleteInvite(x.email,box)};
}
