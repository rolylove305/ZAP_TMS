...
function loadOne(id,src){if(q('#'+id))return;const s=document.createElement('script');s.id=id;s.src=src;document.body.appendChild(s)}
function loadUpgrades(){loadOne('tmsDashboardStable','tms-dashboard-stable.js?v=active-loop-fix-1');loadOne('dashboardFix','dashboard-fix.js?v=6500');loadOne('tmsDocsStable','tms-docs-stable.js?v=5200');loadOne('storageUploadHelper','storage-upload.js?v=paste-upload-1');loadOne('storageManageHelper','storage-manage.js?v=manage-docs-fix-1');loadOne('rateconEmailInboxHelper','ratecon-email-inbox.js?v=1')/* invite-admin removed: invite-only replaced by open signup; user admin lives in admin-users.js */}
...
btn.after(card);q('#saveCompanySettings').onclick=saveSettings;q('#closeCompanySettings').onclick=closeCard;loadSettings()}
